...
class RecommendationService:
    ...
    def build_planner_recommendations(self) -> PlannerRecommendationsResponse:
        recipes = self._list_recipes()
        pantry_items = self._list_pantry_items()
        planned_meals = self._list_planned_meals()
        ...
        repeats = self._repeat_recipes(recipes=recipes, planned_meals=planned_meals)
    ...
    def build_dashboard_recommendations(self) -> DashboardRecommendationsResponse:
        recipes = self._list_recipes()
        pantry_items = self._list_pantry_items()
        planned_meals = self._list_planned_meals()
    ...
    def _list_planned_meals(self) -> list[PlannedMeal]:
        return list(
            self._session.exec(select(PlannedMeal).where(PlannedMeal.user_id == self._user_id)).all()
        )
    ...
    def _recent_plan_counts(self, planned_meals: list[PlannedMeal]) -> dict[str, int]:
